...
import {ACCESS_TOKEN, SUCCESS_CODE,USER_INFO} from "@/store/constant";
    ...
    actions: {
        ...
        Login({commit}, loginInfo) {
            ...
            return new Promise((resolve, reject) => {
                ...
                login({account: account, password: password}).then(r=> {
                    // console.log('拿到的登录数据',res)
                    // let r=res.data
                    // console.log(r.code,SUCCESS_CODE)
                    if (r.code !== SUCCESS_CODE)
                        reject()
                    else {
                        // 登录成功之后更新vuex和缓存中的token
                        let token = r.zooToken
                        storage.set(ACCESS_TOKEN, token)
                        storage.set(USER_INFO,r.data)
